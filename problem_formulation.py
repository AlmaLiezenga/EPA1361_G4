...
def get_model_for_problem_formulation(problem_formulation_id):
    ...
    # Own problem formulation:
        ...
        '''
            ...
            '''
        # uncomment for no optimisation, all actors regarded the same in both cases
        # direction = ScalarOutcome.INFO
        outcomes = []
        for dike in function.dikelist:
            outcomes.append(ScalarOutcome('{}_Expected Annual Damage'.format(dike),
        ...
        '''
           The main goal is to minimize dike investment costs, annual damage and number of deaths only for Zutphen,
           as well as evacuation costs for all actors. The rest of the variables are ignored in the optimization process.
           '''
        outcomes = []
        for dike in function.dikelist:
            if dike == "A.3":
            ...
            # optimize the three outcomes of interest only for Zutphen
            outcomes.append(ScalarOutcome('{}_Dike Investment Costs'.format(dike),
                                          variable_name=['{}_Dike Investment Costs {}'.format(dike, n)
                                                         for n in function.planning_steps],
                                          function=sum_over, kind=direction))
        ...
        # ignore rfr total costs
        outcomes.append(ScalarOutcome('RfR Total Costs',
                                      variable_name=['RfR Total Costs {}'.format(n
                                                                                 ) for n in function.planning_steps],
                                      function=sum_over, kind=ScalarOutcome.INFO))

        # minimize evacuation costs for all
        outcomes.append(ScalarOutcome('Expected Evacuation Costs',
                                      variable_name=['Expected Evacuation Costs {}'.format(n
                                                                                           ) for n in
                                                     function.planning_steps],
        ...
        #pass all of the outcome values to the variable that will be used in the notebooks later
        dike_model.outcomes = outcomes
...
if __name__ == '__main__':
    get_model_for_problem_formulation(10)
