...
def get_model_for_problem_formulation(problem_formulation_id):
    ...
    # But only minimize outcomes for Zutphen right??
    ...
    # Fully disaggregated:
        ...
        for n in function.planning_steps:
            for dike in function.dikelist:
                # only optimise for dike ring 3
                if dike == "A.3":
                    direction = ScalarOutcome.MINIMIZE
                else:
                    direction = ScalarOutcome.INFO
                for entry in ['Expected Annual Damage', 'Dike Investment Costs',
                          'Expected Number of Deaths']:
                    # and only optimise for dike ring 3 for Annual Damage and Number of Deaths
                    if entry == 'Dike Investment Costs':
                        direction ==  ScalarOutcome.INFO
                    else:
                        direction = direction
    ...
    # Own problem formulation:
        ...
        '''
           NEW Self-Specified PF where only these are outcomes our optimised for dike ring 3 (Zutphen)
           - Expected Annual Damage
           - Expected Number of Deaths

           AND the following outcome of interest is optimised for the whole IJsel (since the model does not allow 
           it per dike)
    ...
    # Own problem formulation:
        ...
        '''
           NEW Self-Specified PF where only these are outcomes our optimised for dike ring 3 (Zutphen)
           All other outcomes for all other dike rings are NOT collected!
           - Expected Annual Damage
           - Expected Number of Deaths
        ...
        # always optimised aggregated over all timesteps for all dikes
        outcomes.append(ScalarOutcome('Expected Evacuation Costs',
                                      variable_name=['Expected Evacuation Costs {}'.format(n
                                                                                           ) for n in
                                                     function.planning_steps],
        ...
        '''
            NEW Self-Specified PF where all optimised for all actors -> Change direction to exclude OPTIMISATION
            All outcomes for all dike rings are collected
            Aggregated over time

            '''
        # no optimisation, all actors regarded the samw
        # direction = ScalarOutcome.INFO
        outcomes = []
        for dike in function.dikelist:
            outcomes.append(ScalarOutcome('{}_Expected Annual Damage'.format(dike),
        ...
        for dike in function.dikelist:
            ...
            if dike == "A.3" or dike == "A1" or dike == "A2":
                direction = ScalarOutcome.MINIMIZE
            else:
                direction = ScalarOutcome.INFO
    ...
    elif problem_formulation_id == 10:
    ...
    elif problem_formulation_id == 11:
        '''
           Last Problem Formulation
           - A1 & A2 Damages Minimize
           - A3 Deaths & Damage Minimize
           - All other outcomes not taken into consideration but kept for analysis
           '''
        outcomes = []
        for dike in function.dikelist:
            if dike == "A.3" or dike == "A.2" or dike == "A.1":
                # optimise
                direction_deaths = ScalarOutcome.MINIMIZE
                direction_damage = ScalarOutcome.MINIMIZE

            else:
                direction_deaths = ScalarOutcome.INFO
                direction_damage = ScalarOutcome.INFO


            outcomes.append(ScalarOutcome('{}_Expected Annual Damage'.format(dike),
                                          variable_name=['{}_Expected Annual Damage {}'.format(dike, n)
                                                         for n in function.planning_steps],
                                          function=sum_over, kind=direction_damage))
            # always ignore
            outcomes.append(ScalarOutcome('{}_Expected Number of Deaths'.format(dike),
                                          variable_name=['{}_Expected Number of Deaths {}'.format(
                                              dike, n) for n in function.planning_steps],
    ...
    elif problem_formulation_id == 12:
        '''
           Last Problem Formulation
           - A1 & A2 Damages Minimize
           - A3 Deaths & Damage Minimize
           - All other outcomes not taken into consideration and thus removed from analysis
           '''
        outcomes = []
        direction_deaths = ScalarOutcome.INFO
        direction_damage = ScalarOutcome.INFO
        for dike in function.dikelist:
            ...
            outcomes.append(ScalarOutcome('{}_Expected Number of Deaths'.format(dike),
                                          variable_name=['{}_Expected Number of Deaths {}'.format(
                                              dike, n) for n in function.planning_steps],
                                          function=sum_over, kind=direction_deaths))
...
if __name__ == '__main__':
    get_model_for_problem_formulation(12)